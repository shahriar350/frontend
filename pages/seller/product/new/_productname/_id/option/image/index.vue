<template>
  <div>
    ...
    <div v-for="(option,index) in options" class="card mb-2 mt-3">
      <p class="text-xl">{{ option.name | firstCapital}}</p>
      <div v-for="(opt,optIndex) in option.options" class="ml-5 my-3 border shadow rounded">
        <div class="flex sm:flex-row flex-col-reverse">
          ...
          <transition name="page">
            <div v-if="opt.image" class="md:w-4/12 sm:w-2/12 w-full relative sm:block flex justify-end" >
              ...
              <a-icon @click="removeImage(option.id,opt.id,index,optIndex)" type="delete" class="absolute top-0 right-0 bg-white p-2 shadow rounded" />
            </div>
          </transition>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      removeImage(option,child,index,optIndex){
        this.$swal({
          title: 'Confirmation!',
